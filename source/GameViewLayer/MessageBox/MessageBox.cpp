...
UIMessageBox::UIMessageBox(std::wstring msg, std::wstring title, i32 buttonFlags)
{
	// Initialize dialogs
	...
	int btnWidth = (int) ((float) g_GameApp->GetScreenSize().x * 0.15f);
	int btnHeight = (int) ((float) g_GameApp->GetScreenSize().y * 0.037f);
	int border = (int) ((float) g_GameApp->GetScreenSize().x * 0.043f);

	m_Width = max(msgWidth + 2 * border, btnWidth + 2 * border);
	m_Height = msgHeight + (numButtons * (btnHeight + border)) + (2 * border);

	m_PosX = (g_GameApp->GetScreenSize().x - m_Width) / 2;
	m_PosY = (g_GameApp->GetScreenSize().y - m_Height) / 2;
	...
	if ((buttonFlags == MB_ABORTRETRYIGNORE) || (buttonFlags == MB_CANCELTRYCONTINUE))
	{
		...
		m_UI.AddButton(IDCONTINUE, g_GameApp->GetString(_T("IDS_CONTINUE")).c_str(), iX, iY - (2 * border), btnWidth, btnHeight);
		m_UI.AddButton(IDTRYAGAIN, g_GameApp->GetString(_T("IDS_TRYAGAIN")).c_str(), iX, iY - border, btnWidth, btnHeight);
		m_UI.AddButton(IDCANCEL, g_GameApp->GetString(_T("IDS_CANCEL")).c_str(), iX, iY, btnWidth, btnHeight);
	}
	else if (buttonFlags == MB_OKCANCEL)
	{
		//The message box contains two push buttons: OK and Cancel.
		m_UI.AddButton(IDOK, g_GameApp->GetString(_T("IDS_OK")).c_str(), iX, iY - border, btnWidth, btnHeight);
		m_UI.AddButton(IDCANCEL, g_GameApp->GetString(_T("IDS_CANCEL")).c_str(), iX, iY, btnWidth, btnHeight);
	}
	else if (buttonFlags == MB_RETRYCANCEL)
	{
		//The message box contains two push buttons: Retry and Cancel.
		m_UI.AddButton(IDRETRY, g_GameApp->GetString(_T("IDS_RETRY")).c_str(), iX, iY - border, btnWidth, btnHeight);
		m_UI.AddButton(IDCANCEL, g_GameApp->GetString(_T("IDS_CANCEL")).c_str(), iX, iY, btnWidth, btnHeight);
	}
	else if (buttonFlags == MB_YESNO)
	{
		//The message box contains two push buttons: Yes and No.
		m_UI.AddButton(IDYES, g_GameApp->GetString(_T("IDS_YES")).c_str(), iX, iY - border, btnWidth, btnHeight, 0x59);
		m_UI.AddButton(IDNO, g_GameApp->GetString(_T("IDS_NO")).c_str(), iX, iY, btnWidth, btnHeight, 0x4E);
	}
	else if (buttonFlags == MB_YESNOCANCEL)
	{
		//The message box contains three push buttons: Yes, No, and Cancel.
		m_UI.AddButton(IDYES, g_GameApp->GetString(_T("IDS_YES")).c_str(), iX, iY - (2 * border), btnWidth, btnHeight);
		m_UI.AddButton(IDNO, g_GameApp->GetString(_T("IDS_NO")).c_str(), iX, iY - border, btnWidth, btnHeight);
		m_UI.AddButton(IDCANCEL, g_GameApp->GetString(_T("IDS_CANCEL")).c_str(), iX, iY, btnWidth, btnHeight);
	}
	else //if (buttonFlags & MB_OK)
	{
		// The message box contains one push button: OK. This is the default.
		m_UI.AddButton(IDOK, g_GameApp->GetString(_T("IDS_OK")).c_str(), iX, iY, btnWidth, btnHeight);
	}
}

HRESULT UIMessageBox::OnRestore()
{
	m_UI.SetLocation(m_PosX, m_PosY);
	m_UI.SetSize(m_Width, m_Height);
	return S_OK;
...
HRESULT UIMessageBox::OnRender(f64 time, f32 elapsedTime)
{
	HRESULT hr;
	V(m_UI.OnRender(time));
	return S_OK;
}

LRESULT UIMessageBox::OnMsgProc(AppMsg msg)
{
	return m_UI.MsgProc(msg.m_hWnd, msg.m_uMsg, msg.m_wParam, msg.m_lParam);
}

void UIMessageBox::OnGUIEvent(UINT nEvent, i32 nControlID, CDXUTControl* pControl, void* pUserContext)
{
	PostMessage(g_pApp->GetHwnd(), g_MsgEndModal, 0, nControlID);
}

i32 UIMessageBox::Ask(MessageBox_Questions question)
{
	std::wstring msg;
	std::wstring title;
	int buttonFlags;
