...
void DepthStencilManager::CreateResource(const u32& width, const u32& height, const DXGI_SAMPLE_DESC& msaa, ID3D12Device* device)
{
	auto resourceDesc = Specify(width, height, msaa);
...
void DepthStencilManager::Resize(ID3D12Device* device, u32 width, u32 height, DXGI_SAMPLE_DESC msaa)
{
	CreateResource(width, height, msaa, device);
	CreateView(device);
}
