...
bool ResourceCache::Init()
{
	if (m_file->Open())
	{
		...
		LOG_INFO("Default Resource Loader registered");
...
std::shared_ptr<ResourceHandle> ResourceCache::Load(Resource* r)
{
	...
	std::shared_ptr<IResourceLoader> loader = *std::find_if(
		m_resourceLoaders.begin(),
		m_resourceLoaders.end(),
		[r] (std::shared_ptr<IResourceLoader> testLoader)
		{
			return WildcardMatch(testLoader->GetPattern().c_str(), r->GetName().c_str());
		}
	);
...
int ResourceCache::Preload(const std::string pattern, void (*progressCallback)(int, bool&))
{
	...
	{
		Resource r{ m_file->GetResourceFilename(i) };
...
std::vector<std::string> ResourceCache::Match(const std::string pattern)
{
	...
	for (int i = 0; i < numFiles; ++i)
	{
		std::string name = m_file->GetResourceFilename(i);
...
template<class T>
std::shared_ptr<T> ResourceCache::GetData(std::string filename)
{
	Resource resource(filename);
	std::shared_ptr<ResourceHandle> handle = GetHandle(&resource);  //  loads the file from the zip file
