...
#define EXPORT_FOR_SCRIPT_EVENT(eventClass) \
	public: \
		static ScriptEvent* CreateEventForScript(void) { return new eventClass; }

// typedef std::function<ScriptEvent* ()> CreateEventForScriptFunctionType;
...
protected:
	LuaPlus::LuaObject m_eventData;  //  Lua representation of the data used by the event (typically a table).

public:
	// construction
	inline ScriptEvent(void) : m_eventDataIsValid(false) {}
	...
	static void AddCreationFunction(EventType type, CreateEventForScriptFunctionType func);
...
protected:
	...
	inline virtual void BuildEventData(void) { m_eventData.AssignNil(LuaStateManager::Get()->GetLuaState()); }
	...
	virtual bool BuildEventFromScript(void) { return true; }
};