...
void DX12Engine::Initialize(HWND window, u16 width, u16 height)
{
	...
	// Next steps:
	// 1. upload mesh without command List
	// 2. remove _SwapChain and _DepthStencil
	...
	Command::CreateList(device, _FrameCycle->GetCurrentFrameAllocatorWhenAvailable(), &m_CommandList);
...
void DX12Engine::BuildGeometry()
{
	...
	*/
	...
	_Objects = std::make_unique<RenderObjects>(staticMeshes, dynamicMeshes, objs, device, m_CommandList);
}
...
{
	_resources.Prepare(D3D12_RESOURCE_STATE_RENDER_TARGET);


	//_resources.WaitForGpu();
	//auto cmdListAlloc = _resources.GetCommandAllocator();
